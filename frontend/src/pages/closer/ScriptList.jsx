...
export default function ScriptList({ searchQuery: externalSearchQuery = "", onEditScript }) {
  ...
  const fetchScripts = async () => {
    try {
      ...
      const closerScripts = scriptsArray.filter(
        (script) => script.type?.toLowerCase() === "closer"
      );

      setScripts(closerScripts);
    } catch (err) {
  ...
  const filterScripts = (scriptsList, query) => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="mt-4 text-gray-600 font-medium">Loading closer scripts...</p>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6">
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search closer scripts..."
              value={searchQuery}
      ...
      {filteredScripts.length === 0 ? (
        <div className="text-center py-12 sm:py-16 bg-linear-to-br from-gray-50 to-white rounded-xl sm:rounded-2xl border-2 border-dashed border-gray-200">
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-gray-700 mb-2">No closer scripts found</h3>
          <p className="text-sm text-gray-500 mb-4 sm:mb-6">
            {searchQuery ? "Try adjusting your search" : "No closer scripts available"}
          </p>
          {searchQuery && (
            <button
